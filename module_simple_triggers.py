...
#           (try_end),
 ...
 (1,[
  ...
  (24,
   [
       (display_log_message, "@Checking for recruit departure at {s4}"),
       (try_for_range, ":center_no", villages_begin, villages_end),
            (party_slot_eq, ":center_no", slot_town_lord, "trp_player"),
            ...
            (party_count_members_of_type,":count_recruit_sarranid",":center_no",99),
            (party_count_members_of_type,":count_recruit_khergit",":center_no",63),
            (party_count_members_of_type,":count_recruit_rhodok",":center_no",86),
            (assign,":send_recruit",0),
            (try_begin),
                (gt,":count_recruit_swadia",0),
                (assign,":send_recruit",1),
                (gt,":count_recruit_vaegir",0),
                (assign,":send_recruit",1),
                (gt,":count_recruit_nord",0),
                (assign,":send_recruit",1),
                (gt,":count_recruit_sarranid",0),
                (assign,":send_recruit",1),
                (gt,":count_recruit_khergit",0),
                (assign,":send_recruit",1),
                (gt,":count_recruit_rhodok",0),
                (assign,":send_recruit",1),
            (try_end),
            (try_begin),
                (eq,":send_recruit",1),
                (display_log_message, "@sending recruit is a go at {s4}"),
                (set_spawn_radius,0),
                (spawn_around_party, ":center_no", "pt_fresh_recruits"),
                (assign, ":new_party", reg0),
                (party_set_ai_object,":new_party",":recruitment_destination"),
                (party_set_ai_behavior,":new_party",ai_bhvr_travel_to_party),
                (party_set_slot,":new_party",slot_party_type,spt_recruits),
                (party_set_faction,":new_party",":recuitment_faction"),
                (party_add_members,":new_party",35,":count_recruit_swadia"),
                (party_add_members,":new_party",47,":count_recruit_vaegir"),
                (party_add_members,":new_party",73,":count_recruit_nord"),
                (party_add_members,":new_party",99,":count_recruit_sarranid"),
                (party_add_members,":new_party",63,":count_recruit_khergit"),
                (party_add_members,":new_party",86,":count_recruit_rhodok"),

                (party_remove_members,":center_no",35,":count_recruit_swadia"),
                (party_remove_members,":center_no",47,":count_recruit_vaegir"),
                (party_remove_members,":center_no",73,":count_recruit_nord"),
                (party_remove_members,":center_no",99,":count_recruit_sarranid"),
                (party_remove_members,":center_no",63,":count_recruit_khergit"),
                (party_remove_members,":center_no",86,":count_recruit_rhodok"),
                (display_log_message, "@New Recruits are departing from {s4}"),
            (else_try),
                (display_log_message, "@No new Recruits {s4}"),
            (try_end),
       (try_end),
  ...
  (0.5,[(try_for_parties, ":party_no"),
             (party_slot_eq,":party_no", slot_party_type, spt_recruits),
             (get_party_ai_object,":destination",":party_no"),
             (store_distance_to_party_from_party, ":distance_to_destination", ":party_no", ":destination"),
             (try_begin),
                 (le, ":distance_to_destination", 0),
                 ...
                 (try_for_range, ":stack_no", 0, ":stacks"),
                    ...
                    (party_add_members, ":destination", ":troop_id", ":size"),
                 (try_end),


                 (str_store_party_name_link, s13, ":destination"),
                 (store_party_size, reg10, ":party_no"),
                 (display_log_message, "@{reg10} recruits has arrived at {s13}.", 0x00FF00),
                 (remove_party, ":party_no"),
             (try_end),
